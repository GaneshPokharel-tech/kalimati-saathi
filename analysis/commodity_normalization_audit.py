"""Audit commodity name inconsistencies across the price history."""
...
COUNTS_OUT = ROOT / "data/processed/commodity_name_counts.csv"
EXACT_OUT = ROOT / "data/processed/commodity_normalization_exact_groups.csv"
FUZZY_OUT = ROOT / "data/processed/commodity_normalization_fuzzy_pairs.csv"
...
def load_history() -> pd.DataFrame:
    if DB_PATH.exists():
        ...
        try:
            df = pd.read_sql_query(
                "SELECT commodity, unit, scrape_date_bs FROM price_history", conn
            )
            if not df.empty:
                return df
        finally:
    ...
    if HISTORY_CSV.exists():
        return pd.read_csv(
            HISTORY_CSV, usecols=["commodity", "unit", "scrape_date_bs"]
        ).copy()
...
def main() -> None:
    ...
    exact_groups = []
    for (unit, canonical_key), group in counts_df.groupby(
        ["unit", "canonical_key"], dropna=False
    ):
    ...
    fuzzy_rows = []
    for unit, group in counts_df.groupby("unit", dropna=False):
        records = group[
            ["commodity", "canonical_key", "observations", "unique_dates"]
        ].to_dict("records")
    ...
    print(f"Counts file: {COUNTS_OUT}")
    print(f"Exact-group file: {EXACT_OUT}")
    print(f"Fuzzy-pair file: {FUZZY_OUT}")
    print()
    print(f"Unique commodity-unit pairs: {len(counts_df)}")
    print(f"Exact canonical collision groups: {len(exact_df)}")
    print(f"Fuzzy candidate pairs: {len(fuzzy_df)}")
    print()

    print("Top exact collision groups:")
    print(exact_df.head(15).to_string(index=False) if len(exact_df) > 0 else "None")

    print()
    print("Top fuzzy candidate pairs:")
    print(fuzzy_df.head(20).to_string(index=False) if len(fuzzy_df) > 0 else "None")
