...
STRONGER_HISTORICAL_END = pd.Timestamp("2026-01-31")
...
def add_history_confidence(df: pd.DataFrame) -> pd.DataFrame:
    ...
    low_mask = effective_date.between(LOW_CONFIDENCE_START, LOW_CONFIDENCE_END, inclusive="both")
    medium_mask = effective_date.between(MEDIUM_CONFIDENCE_START, MEDIUM_CONFIDENCE_END, inclusive="both")
    stronger_mask = effective_date.between(STRONGER_HISTORICAL_START, STRONGER_HISTORICAL_END, inclusive="both")
    ...
    df["history_confidence_rank"] = df["history_confidence_band"].map(CONFIDENCE_RANK_MAP).fillna(0).astype(int)

    df["is_low_confidence_history"] = df["history_confidence_band"].eq("low_confidence_historical")
    df["is_medium_confidence_history"] = df["history_confidence_band"].eq("medium_confidence_historical")
    ...
    df["is_default_model_window"] = df["history_confidence_band"].isin(["stronger_historical", "current_live"])
...
def confidence_band_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["history_confidence_rank"] = summary["history_confidence_band"].map(CONFIDENCE_RANK_MAP).fillna(0).astype(int)
