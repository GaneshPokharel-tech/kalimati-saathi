...
print(
    latest_df.sort_values("avg_price", ascending=False)[["commodity", "unit", "avg_price"]]
    .head(10)
...
if len(date_order_df) < 2:
    print("Trend comparison not available yet because history has only one date.")
else:
    current_date = date_order_df.iloc[-1]["scrape_date_bs"]
    previous_date = date_order_df.iloc[-2]["scrape_date_bs"]

    current_df = df[df["scrape_date_bs"] == current_date][["commodity", "avg_price"]].rename(
        columns={"avg_price": "current_avg_price"}
    )
    previous_df = df[df["scrape_date_bs"] == previous_date][["commodity", "avg_price"]].rename(
