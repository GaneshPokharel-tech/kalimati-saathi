...
def build_status():
    ...
    if HISTORY_PATH.exists():
        ...
        if len(df) > 0:
            ...
            status["latest_history_bs_date"] = None if pd.isna(latest_row["scrape_date_bs"]) else str(latest_row["scrape_date_bs"])

            requested_ad = None if pd.isna(latest_row["requested_date_ad"]) else str(latest_row["requested_date_ad"]).strip()
            if requested_ad:
                status["latest_history_ad_date"] = requested_ad
            else:
                latest_sort_date = latest_row["sort_date"]
                status["latest_history_ad_date"] = None if pd.isna(latest_sort_date) else pd.Timestamp(latest_sort_date).strftime("%Y-%m-%d")
...
def main():
    ...
    build_status()
    print("Saved pipeline status:", STATUS_PATH, flush=True)
